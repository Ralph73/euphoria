...
import { updatePromotion, uploadOutfitImage, type Promotion } from '@/lib/actions';
...
interface EditOutfitModalProps {
    ...
    onClose: () => void;
    onSuccess: () => void;
    outfit: Promotion | null;
}

export default function EditOutfitModal({ isOpen, onClose, onSuccess, outfit }: EditOutfitModalProps) {
    ...
    useEffect(() => {
        if (outfit) {
            setDayOfWeek(outfit.day_of_week);
            setTheme(outfit.theme);
            setName(outfit.name);
            setTeaser(outfit.teaser || '');
            setCtaText(outfit.cta_text);
    ...
    const handleRemoveImage = () => {
        if (imagePreview && imagePreview !== outfit?.image_url) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        let finalImageUrl = imagePreview === outfit?.image_url ? outfit.image_url : null;
        ...
        const result = await updatePromotion(outfit.id, {
        ...
        if (result.success) {
            await Swal.fire({
                ...
                text: 'El outfit se ha actualizado correctamente',
                timer: 2000,
            ...
            await Swal.fire({
                icon: 'error',
                title: 'Error',
                text: result.error || 'Error al actualizar el outfit',
                background: '#1f2937',
                color: '#ffffff',
                customClass: { popup: 'rounded-2xl' }
            });
